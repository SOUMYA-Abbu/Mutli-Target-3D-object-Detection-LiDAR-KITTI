...
db_sampler = dict(
    ...
    points_loader=dict(
        coord_type='LIDAR', load_dim=4, type='LoadPointsFromFile', use_dim=4),
    prepare=dict(
...
launcher = 'none'
load_from = '/raid/cs21resch15003/mmdetection3d/checkpoint/dual_attention_expt/epoch_150.pth'
log_level = 'INFO'
log_processor = dict(
    by_epoch=True,
...
model = dict(
    ...
    voxel_encoder=dict(
        ...
        type='VoxelDualAttentionEncoder',
        voxel_size=[
            0.16,
            0.16,
            4,
...
test_evaluator = dict(
    ...
    metric='bbox',
    pklfile_prefix='VoxAttenEncoder/res_dir/',
    type='KittiMetric')
test_pipeline = [
    dict(channel_order='rgb', color_type='color', type='LoadImageFromFile'),
    dict(coord_type='LIDAR', load_dim=4, type='LoadPointsFromFile', use_dim=4),
    dict(
        flip=False,
        img_scale=(
...
val_evaluator = dict(
    ann_file='data/kitti/kitti_infos_val.pkl',
    metric='bbox',
    pklfile_prefix='WORK_DIR/val_res',
    type='KittiMetric')
val_pipeline = [
    dict(coord_type='LIDAR', load_dim=4, type='LoadPointsFromFile', use_dim=4),
    dict(type='LoadAnnotations3D', with_bbox_3d=True, with_label_3d=True),
...
work_dir = './output/test/kitti_dist_test'
workflow = [
    (
        'train',
        1,
